...
decodeur2 = Dense(Layer1,activation = 'sigmoid',name = '2_decodeur')(encodeur1)

autoencodeur = Model(input_img, decodeur2)
autoencodeur.summary()

#encodeur
encodeur = Model(input_img , encodeur1)
encodeur.summary()
...
decoder = Model(input_reduit,decoder_layer1(input_reduit))
decoder.summary()
...
history = autoencodeur.fit(image1CoupeLigne,image1CoupeLigne,
                epochs=10000,
                batch_size=100,#nombre de fois ou l'on calcule le gradient pour faire un moyenne avant utilisation
